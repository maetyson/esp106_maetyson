...
for(i in 2:6){
  ...
  temp_data$city <- city_df$city[i]
  temp_data$gauge <- city_df$gauge[i]
...
```{r question 3.1}
tidedata$Date <- paste(tidedata$Year,tidedata$Month,"01",sep="-")
...
```{r question 3.2}
ymd(tidedata$Date)
tidedata$Date<- as.Date(tidedata$Date)
```
...
library(ggplot2)
ggplot(tidedata, aes(x=Date,y=MHHW,col=city))+
  geom_line()+
geom_smooth(aes(x=Date,y=MHHW),method="lm",inherit.aes=FALSE, col = "black")
```
...
library(ggplot2)
ggplot(tidedata, aes(x=Date,y=MHHW))+
...
```{r question 6}
boxplot(Highest~city, data=tidedata)
...
tidedata[which.max(tidedata$Highest),]
```
...
Hurricane Sandy happened on 29-10-2012
...
mod <- lm(MHHW~Date,data=tidedata)
sum_mod<-summary(mod)
sum_mod$coefficient
```
