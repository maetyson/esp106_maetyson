...
```{r question 2.1}
unzip("Week_4_export.zip")
##unzipping folder

data <- list.files(path="Data", full.names =TRUE)
##looking at file names, telling R to look into Data folder specifically 
...
```{r question 2.2}
##whole list of cites
tidedata <- lapply(data, read.csv)
##now we use function to apply read.cvs function to each file in Data folder and create new objects called files
##lappy applies a function to a list of things

##one city in file
##tidedata <- read.csv(data[1])
tidedata
```
...
##tidedata$city <- city_df$city[1]
##tidedata$gauge <- city_df$gauge[1]
...
```{r question 2.4}
for(i in 1:length(city_df$city)){
  tidedata[[1]]$city <- rep((city_df$city)[i], nrow(tidedata[[1]])), tidedata[[1]]$gauge <- rep((city_df$gauge)[i], nrow(tidedata[[1]]))

}

sapply(tidedata, ncol)
combine_tides <- do.call(rbind, tidedata)
```
